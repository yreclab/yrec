...
C     
C    Edits in 2025-2026 to make input and export a little bit easier
C    and intuitive. 
      ...
      real moles,molesz
      real(4) :: zz
      integer :: num_args, io_status
      character(len=32) :: arg_str
      character(len=15) :: zz_str
...
c     --- Check for command line arguments ---
      ...
      if (num_args > 0) then
        ...
        read(arg_str, *, iostat=io_status) zz
        
      if (io_status /= 0) then
          write (*,*) 'Error: Invalid Z value provided.'
          stop
      end if
        write (*,'("Z read from command line: ",f15.9)') zz
        
      else
        write (*,'("type Z:  ",$)')
c       Read the user's exact input as a string first
        read (*,'(a)') arg_str
c       Convert that string into the real number zz
        read (arg_str, *) zz
        write (*,'(f15.9)') zz
      end if

c     Now, glue the exact string the user typed onto the filename!
      tabfilez = 'EOSOPAL06Z' // trim(adjustl(arg_str))
