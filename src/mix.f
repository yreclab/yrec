...
c GES 6/15 INCLUDED COMMON BLOCK FOR NEW DIFFUSION ROUTINES.
     ...
     *     HDO(JSON),HCOMPP(17,JSON),HSS(JSON),LCO(JSON),LCZO(JSON),TEFFLO,MO
...
C ADDED FOR MASS ACCRETION
     ...
     * HS2(JSON),HT(JSON),LC(JSON),HCOMP(17,JSON),HQPR(JSON), HR(JSON)
      DIMENSION HR1(JSON),HR2(JSON),HR3(JSON),HR4(JSON),
     * HR5(JSON),HR6(JSON),HR7(JSON),HR8(JSON),HR9(JSON),HR10(JSON),
     * HR11(JSON),HR12(JSON),HR13(JSON),HF1(JSON),HF2(JSON),HF3(JSON),HF4(JSON),
     * XSUM(17)
...
C FIND BURNING RATES (HR1- HR13,HF1,HF2,HF3,HF4).
...
C      *        XO16,XO17,XO18,XH2,XLI6,XLI7,XBE9,I,HR1,HR2,HR3,HR4,  ! KC 2025-05-31
     ...
     *        HR5,HR6,HR7,HR8,HR9,HR10,HR11,HR12,HR13,HF1,HF2,HF3,HF4)
