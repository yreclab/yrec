...
C       DIMENSION HCOMP(17,JSON),HD(JSON),HS2(JSON),HT(JSON),HS1(JSON)  ! KC 2025-05-31
      DIMENSION HCOMP(17,JSON),HS2(JSON),HT(JSON),HS1(JSON)
      DIMENSION RATLI6(JSON),RATLI7(JSON),RATBE9(JSON),RATB10(JSON),RATB11(JSON)
      DIMENSION HR(JSON)
C       DATA TLIM/6.0D0/
      SAVE
C DETERMINE LITHIUM, BERYLLIUM, AND BORON BURNING.
...
C RADIATIVE INTERIOR.
      ...
      DO I = 1,MINJ-1
         ...
         IF(RB10(I).LE.1.0D-32 .OR. RB100(I).LE.1.0D-32)GOTO 50
         IF(RB11(I).LE.1.0D-32 .OR. RB110(I).LE.1.0D-32)GOTO 50
     ...
     *   .AND.HCOMP(17,I).LT.1.0D-24)GOTO 50
         ...
         TB10 = 0.5D0*(LOG(RB10(I)) + LOG(RB100(I)))
         TB11 = 0.5D0*(LOG(RB11(I)) + LOG(RB110(I)))
         RATLI6(I) = DELTS*EXP(TLI6)
         RATLI7(I) = DELTS*EXP(TLI7)
         RATBE9(I) = DELTS*EXP(TBE9)
         RATB10(I) = DELTS*EXP(TB10)
         RATB11(I) = DELTS*EXP(TB11)
...
C SKIP IF WHOLE CZ IS BELOW THE BURNING THRESHOLD.
      ...
      IF(RB100(JENV0).LE.1.0D-32.OR.RB10(JENV).LE.1.0D-32)GOTO 200
      IF(RB110(JENV0).LE.1.0D-32.OR.RB11(JENV).LE.1.0D-32)GOTO 200
...
C TIME-AVERAGED RATES USING LINEAR INTERPOLATION IN THE LOG.
         ...
         CZB10 = 0.5D0*(BB10+BB100)
         CZB11 = 0.5D0*(BB11+BB110)
         DLI6 = DELTS*EXP(CZLI6)
         DLI7 = DELTS*EXP(CZLI7)
         DBE9 = DELTS*EXP(CZBE9)
         DB10 = DELTS*EXP(CZB10)
         DB11 = DELTS*EXP(CZB11)
...
C***REMEMBER TO ADD FAILSAFES FOR LARGE DEPLETION***
         ...
         IF(HCOMP(15,I).LT.1.0D-24)HCOMP(15,I)=0.0D0
         HCOMP(16,I) = HCOMP(16,I)/EXP(DB10)
         IF(HCOMP(16,I).LT.1.0D-24)HCOMP(16,I)=0.0D0
         HCOMP(17,I) = HCOMP(17,I)/EXP(DB11)
         IF(HCOMP(17,I).LT.1.0D-24)HCOMP(17,I)=0.0D0
      END DO
 200  CONTINUE
      RETURN
