...
C LMB 05/26 ADDED NTM/NGM FOR KTTAU=6 (MARCS)
...
C JNT 6/14 ADD FOR KURUCZ/CASTELLI 2004 ATMOSPHERES
     ...
     *              ATMGLC(NGC)
C LMB 05/26 ADD FOR MARCS ATMOSPHERES
...
C KURUCZ-CASTELLI ATMOSPHERES
         IF(LSTCH) LSTATM=.FALSE.
         CALL KCSURFP(TEFFL,GL,LPRT.AND.LSTATM)
         GOTO 200
C LMB 05/26 ADD KTTAU
C GET PRESSURE AT T=Teff BY INTERPOLATION IN TABLE ATMPLM.
      ELSE IF (KTTAU .EQ. 6) THEN
C MARCS ATMOSPHERES
         IF(LSTCH) LSTATM=.FALSE.
         CALL MSURFP(TEFFL,GL,LPRT.AND.LSTATM)
         GOTO 200
