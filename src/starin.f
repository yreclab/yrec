...
C DBG 7/92 COMMON BLOCK ADDED TO COMPUTE DEBYE-HUCKEL CORRECTION.
     ...
     *HCOMP(17,JSON),OMEGA(JSON),HS1(JSON),HS2(JSON),QIW(JSON),
...
C LC 2026: Added B10, B11
      IF(LISOTOPE)THEN
         ...
         DO J = 1,M
            ...
            HCOMP(17,J)=XB11_INI
         END DO
         WRITE(*,593)(TESTMIX(K),K=4,17),(HCOMP(K,1),K=4,17)
         WRITE(ISHORT,593)(TESTMIX(K),K=4,17),(HCOMP(K,1),K=4,17)
 593     FORMAT('CNO ISOTOPES AND LIGHT ELEMENTS CHANGED IN ',
     *        'STARIN. OLD HE3 C12 C13 N14 N15 O16 O17 O18 H2 LI6 ',
     *         'LI7 BE9 B10 B11',12E12.4,' NEW ',12E12.4)
      ENDIF
 602  CONTINUE
 3000 CONTINUE
