...
C STRUCTURE CALCULATIONS - SET ITDIF1 GREATER THAN 1
     ...
     *      IOVIM, LOVSTC, LOVSTE, LOVSTM, LSEMIC, LADOV, LOVMAX
      ...
      COMMON/PREVCZ/RBASE0,BLI6OLD,BLI7OLD,BBE9OLD,BB10OLD,BB11OLD,JENVOS
...
C 7/91 FLAG TO TOGGLE BETWEEN NEW/OLD RATES ADDED.
     ...
     *              HCOMPP(17,JSON),HSS(JSON),LCO(JSON),LCZO(JSON),TEFFLO,MO
C MHP 9/94
      COMMON/ROTPRT/LPRT0
...
C DBG 4/26/94 Tired of not have access to current age of model so...
     ...
     *HCOMP(17,JSON),HS1(JSON),HS2(JSON),HHC(JSON),
...
C     MHP 10/24 FLAG FOR END OF RUN
      LOGICAL*4 LEND_KIND
C JVS 02/12
      COMMON/STCH/HCOMPF(17,JSON),HRF(JSON),HPF(JSON),HDF(JSON),
     * HSF(JSON),HTF(JSON)
...
C MHP 05/02 ADDED FOR ITERATION BETWEEN ROTATION AND STRUCTURE CALCULATION
      COMMON/ORIGSTART/HJM0(JSON),HCOMPP0(17,JSON)
...
C
     ...
     2 HR12(JSON),HR13(JSON),HF1(JSON),HF2(JSON),HF3(JSON),HF4(JSON)
...
C      *XO18,XH2,XLI6,XLI7,XBE9,I,HR1,HR2,HR3,HR4,HR5,HR6,HR7,  ! KC 2025-05-31
     ...
     *HR8,HR9,HR10,HR11,HR12,HR13,HF1,HF2,HF3,HF4)
C BE7 MASS FRACTION.
